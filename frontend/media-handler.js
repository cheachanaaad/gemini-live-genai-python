...
class MediaHandler {
  constructor() {
    ...
    this.voiceActiveHangover = 0;
    this.silentKeepAliveCounter = 0;
  }
  ...
  async startAudio(onAudioData) {
    ...
    try {
      ...
      this.audioWorkletNode.port.onmessage = (event) => {
        if (this.isRecording) {
          const rms = this.calculateRms(event.data);
          const hasSpeech = rms >= 0.006;
          if (hasSpeech) {
            this.voiceActiveHangover = 10;
            this.silentKeepAliveCounter = 0;
          } else if (this.voiceActiveHangover > 0) {
            this.voiceActiveHangover -= 1;
          }

          if (!hasSpeech && this.voiceActiveHangover === 0) {
            this.silentKeepAliveCounter += 1;
            if (this.silentKeepAliveCounter % 8 !== 0) {
              return;
            }
          }
  ...
  async startVideo(videoElement, onFrame, options = {}) {
    try {
      const constraints = {
        video: {
          width: { ideal: 1280 },
      ...
      this.videoFrameCallback = onFrame;
